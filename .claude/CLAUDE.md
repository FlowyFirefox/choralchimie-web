...
`# | Langue | Track Name | Artist Name(s) | Instrument | Difficulté vocale | BPM | Tonalité | Key (EN) | Capo (guitare) | Genres | Validée Choraoké | Spotify Link | Lien UltimateGuitar | Notes`

### Flux mise à jour répertoire
```
1. Modifier le Sheet (ajouter/valider lignes)
2. python scripts/sync_repertoire.py
3. Script régénère choraoke-app/index.html
...
1. `scripts/sync_repertoire.py` (pipeline Sheet → HTML)
2. Bouton ➕ dans `choraoke-app/index.html`
3. `accueil/index.html`
4. `hub-chanteurs/index.html`
5. `hub-musiciens/index.html`
6. `planning/index.html`
7. `formulaires/musicien.html`
